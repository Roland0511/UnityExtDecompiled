	...
	public class TimelinePlayable : PlayableBehaviour
	{
		...
		private void AllocateDefaultTracks(PlayableGraph graph, Playable timelinePlayable, IList<TrackAsset> tracks, GameObject go)
		{
			if (!Application.get_isPlaying())
			{
				...
				if (!(component == null))
				{
					HashSet<Animator> hashSet = null;
					foreach (TrackAsset current in tracks)
					{
						Animator animator = AnimationDefaultTrack.GetAnimator(current, component);
						if (!(animator == null) && animator.get_isHuman())
						{
							if (hashSet == null)
							{
								hashSet = new HashSet<Animator>();
							}
							if (!hashSet.Contains(animator))
							{
								hashSet.Add(animator);
								AnimationDefaultTrack.AddDefaultHumanoidTrack(graph, timelinePlayable, animator);
							}
						}
					}
				}
